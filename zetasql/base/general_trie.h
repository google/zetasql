...
class GeneralTrieImpl {
 public:
  ...
  // Returns the data associated with s in the trie, or NullValuePolicy::Null()
  // if s is not in the trie.
  const T& GetData(const char* s) const;

  // Returns a reference to the data associated with s in the trie, or
  // NullValuePolicy::Null() if s is not in the trie.
  T& GetData(const char* s);

  // If "s" is present in the trie, updates the data associated with s and
  ...
  // the first n characters of s and sets *chars_matched to n.  is_terminator
  ...
  const T& GetDataForMaximalPrefix(const char* s, int* chars_matched,
                                   const bool* is_terminator) const;

  // Same as GetDataForMaximalPrefix but takes in the length of the input
  // so that we don't need to compute it. This is useful for strings that are
  // long and computing strlen everytime is a waste.
  const T& GetDataForMaximalPrefixWithLen(const char* s, int slen,
                                          int* chars_matched,
                                          const bool* is_terminator) const;
  ...
  void PreorderTraverseAllMatchingStringsDepth(const char* s, int len,
                                               Traverser* traverser,
                                               int depth) const {
    TraverseAllMatchingStrings(s, len, traverser, depth, true);
  }
  ...
  void PostorderTraverseAllMatchingStrings(const char* s, int len,
                                           Traverser* traverser) const {
    PostorderTraverseAllMatchingStringsDepth(s, len, traverser, -1);
  }
  void PostorderTraverseAllMatchingStringsDepth(const char* s, int len,
                                                Traverser* traverser,
                                                int depth) const {
    TraverseAllMatchingStrings(s, len, traverser, depth, false);
  }
...
template <class T, class NullValuePolicy>
void GeneralTrieImpl<T, NullValuePolicy>::Insert(const char* s, const T& data) {
  int diff;
  int slen = strlen(s);

  if (slen == 0) {  // we're at our leaf
    data_ = data;
    return;
  }

  // Break up compression if we have to
  if (!comppath_.empty() &&         // easy out: no compression
      (comppath_.size() >= slen ||  // bad: compression too long
       memcmp(comppath_.c_str(), s, comppath_.size()))) {  // doesn't match
    for (diff = 0; s[diff] != '\0'; diff++)                // pos of mismatch
      if (comppath_[diff] != s[diff]) break;
    if (diff == slen) {
  ...
  // been empty), and insert the rest of s as a child.  Otherwise we follow
  // the path based on the first char of s (creating a child node first if
  // need be).
  int i;
  for (i = min_next_; i < max_next_; i++)  // does root have any kids?
    if (Next(i)) break;
  if (i == max_next_) {             // no? take over comppath
    comppath_.assign(s, slen - 1);  // w/o kids, comppath was empty
    NodeT* next = SetNext(s[slen - 1], new NodeT());  // one kid now
    next->Insert(s + slen, data);
  } else {  // has a kid, and a comppath
    NodeT* nextnode = Next(s[0]);
    if (!nextnode)  // but not the right kid
      nextnode = SetNext(s[0], new NodeT());
    nextnode->Insert(s + 1, data);  // continue down the trie
  }
}
...
//    Returns the data associated with s in the trie, or
//    NullValuePolicy::Null() if s is not in the trie.
// ----------------------------------------------------------------------

template <class T, class NullValuePolicy>
const T& GeneralTrieImpl<T, NullValuePolicy>::GetData(const char* s) const {
  return *GetDataPtr(s);
}
...
template <class T, class NullValuePolicy>
T& GeneralTrieImpl<T, NullValuePolicy>::GetData(const char* s) {
  return *const_cast<T*>(GetDataPtr(s));
}

template <class T, class NullValuePolicy>
const T* GeneralTrieImpl<T, NullValuePolicy>::GetDataPtr(const char* s) const {
  const NodeT* node = this;
  int slen = strlen(s);
  int next_pos = 0;

  while (node) {
    // Return if we've reached the end of s.  Note that node->data_
    ...
    if (!node->comppath_.empty() &&  // comppath to match
        (node->comppath_.size() >= (slen - next_pos) ||
         memcmp(node->comppath_.c_str(), s + next_pos,
                node->comppath_.size())))  // or it doesn't match
      return &null_value_instance_;        // we're done

    // follow first char after comppath
    next_pos += node->comppath_.size();
    //    cerr << "comp: " << node->comppath_ << endl;
    node = node->Next(s[next_pos]);
    //    cerr << next_pos << ": " << s[next_pos] << endl;
    next_pos++;
  }
...
template <class T, class NullValuePolicy>
bool GeneralTrieImpl<T, NullValuePolicy>::SetData(const char* s,
                                                  const T& data) {
  const T* temp_ptr = GetDataPtr(s);
...
//    with the first n characters of s and sets *chars_matched to n.
...
const T& GeneralTrieImpl<T, NullValuePolicy>::GetDataForMaximalPrefix(
    const char* s, int* chars_matched, const bool* is_terminator) const {
  int slen = strlen(s);
  return GetDataForMaximalPrefixWithLen(s, slen, chars_matched, is_terminator);
}

// ----------------------------------------------------------------------
// GeneralTrieImpl<T, NullValuePolicy>::GetDataForMaximalPrefixWithLen()
//    Same as GetDataForMaximalPrefix but takes in the length of the input
//    so that we don't need to compute it. This is useful for strings that are
//    long and computing strlen everytime is a waste.
// ----------------------------------------------------------------------
template <class T, class NullValuePolicy>
const T& GeneralTrieImpl<T, NullValuePolicy>::GetDataForMaximalPrefixWithLen(
    const char* s, int slen, int* chars_matched,
    const bool* is_terminator) const {
  const NodeT* node = this;
  int next_pos = 0;
...
class TrieExtractor : public GeneralTrieImpl<T, NullValuePolicy>::Traverser {
 public:
  ...
  virtual void Process(const std::string& s, const T& data) {
...
void GeneralTrieImpl<T, NullValuePolicy>::GetAllMatchingStrings(
    const char* s, int len, std::vector<TrieData>* outdata) const {
...
void GeneralTrieImpl<T, NullValuePolicy>::TraverseAllMatchingStrings(
  ...
  while (node) {
    ...
    const int len_to_compare =
        std::min(static_cast<int>(node->comppath_.size()), (len - next_pos));
    if (memcmp(node->comppath_.data(), s + next_pos, len_to_compare) != 0) {
      // mismatch found
      return;
    }

    if ((len - next_pos) <= node->comppath_.size()) {
