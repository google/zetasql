...
inline static uint64_t BN_ext_get_uint64(const BIGNUM* bn) {
  ...
  if (!BN_get_u64(bn, &u64)) {
    ZETASQL_DCHECK(false) << "BN has " << BN_num_bits(bn) << " bits";
    return 0;
  }
  return u64;
}
...
inline static void BN_ext_set_uint64(BIGNUM* bn, uint64_t v) {
#if BN_BITS2 == 64
  ZETASQL_CHECK(BN_set_word(bn, v));
#else
  static_assert(BN_BITS2 == 32, "at least 32 bit openssl build needed");
  ZETASQL_CHECK(BN_set_word(bn, static_cast<uint32_t>(v >> 32)));
  ZETASQL_CHECK(BN_lshift(bn, bn, 32));
  ZETASQL_CHECK(BN_add_word(bn, static_cast<uint32_t>(v)));
#endif
}
...
// Requires that BIGNUM fits into 64 bits.
inline static uint64_t BN_ext_get_uint64(const BIGNUM* bn) {
  ZETASQL_DCHECK_LE(BN_num_bytes(bn), sizeof(uint64_t));
#if BN_BITS2 == 64
...
static int BN_ext_count_low_zero_bits(const BIGNUM* bn) {
  ...
  ZETASQL_CHECK_EQ(BN_bn2lebinpad(bn, bytes.data(), bytes.size()), bytes.size());
...
ExactFloat::ExactFloat(int v) {
  ...
  ZETASQL_CHECK(BN_set_word(bn_.get(), abs(v)));
  bn_exp_ = 0;
  Canonicalize();
}
...
double ExactFloat::ToDoubleHelper() const {
  ZETASQL_DCHECK_LE(BN_num_bits(bn_.get()), kDoubleMantissaBits);
  if (!is_normal()) {
...
ExactFloat ExactFloat::RoundToMaxPrec(int max_prec, RoundingMode mode) const {
  ...
  ZETASQL_DCHECK_GE(max_prec, 2);
  ZETASQL_DCHECK_LE(max_prec, kMaxPrec);
...
ExactFloat ExactFloat::RoundToPowerOf2(int bit_exp, RoundingMode mode) const {
  ZETASQL_DCHECK_GE(bit_exp, kMinExp - kMaxPrec);
  ZETASQL_DCHECK_LE(bit_exp, kMaxExp);
  ...
  if (mode == kRoundTowardZero) {
    ...
    ZETASQL_DCHECK_EQ(mode, kRoundTiesToEven);
  ...
  ZETASQL_CHECK(BN_rshift(r.bn_.get(), bn_.get(), shift));
  if (increment) {
    ZETASQL_CHECK(BN_add_word(r.bn_.get(), 1));
  }
  r.sign_ = sign_;
  r.Canonicalize();
  return r;
...
std::string ExactFloat::ToStringWithMaxDigits(int max_digits) const {
  ZETASQL_DCHECK_GT(max_digits, 0);
  if (!is_normal()) {
...
int ExactFloat::GetDecimalDigits(int max_digits, std::string* digits) const {
  ...
  if (bn_exp_ >= 0) {
    ...
    ZETASQL_CHECK(BN_lshift(bn, bn_.get(), bn_exp_));
    bn_exp10 = 0;
  } else {
    // Set bn = bn_ * (5 ** -bn_exp_) and bn_exp10 = bn_exp_.  This is
    // equivalent to the original value of (bn_ * (2 ** bn_exp_)).
    BIGNUM* power = BN_new();
    ZETASQL_CHECK(BN_set_word(power, -bn_exp_));
    ZETASQL_CHECK(BN_set_word(bn, 5));
    BN_CTX* ctx = BN_CTX_new();
    ZETASQL_CHECK(BN_exp(bn, bn, power, ctx));
    ZETASQL_CHECK(BN_mul(bn, bn, bn_.get(), ctx));
    BN_CTX_free(ctx);
    BN_free(power);
    bn_exp10 = bn_exp_;
  }
  ...
  ZETASQL_DCHECK(all_digits != nullptr);
  BN_free(bn);
  ...
  ZETASQL_DCHECK_LE(digits->size(), max_digits);
...
ExactFloat ExactFloat::SignedSum(int a_sign, const ExactFloat* a,
  ...
  ExactFloat r;
  if (a->bn_exp_ > b->bn_exp_) {
    ZETASQL_CHECK(BN_lshift(r.bn_.get(), a->bn_.get(), a->bn_exp_ - b->bn_exp_));
    a = &r;  // The only field of "a" used below is bn_.
  }
  r.bn_exp_ = b->bn_exp_;
  if (a_sign == b_sign) {
    ZETASQL_CHECK(BN_add(r.bn_.get(), a->bn_.get(), b->bn_.get()));
    r.sign_ = a_sign;
  } else {
    ...
    ZETASQL_CHECK(BN_sub(r.bn_.get(), a->bn_.get(), b->bn_.get()));
...
void ExactFloat::Canonicalize() {
  ...
  if (my_exp < kMinExp || BN_is_zero(bn_.get())) {
    ...
    ZETASQL_DCHECK(!BN_is_zero(bn_.get()));
    int shift = BN_ext_count_low_zero_bits(bn_.get());
    if (shift > 0) {
      ZETASQL_CHECK(BN_rshift(bn_.get(), bn_.get(), shift));
      bn_exp_ += shift;
    }
  }
...
ExactFloat operator*(const ExactFloat& a, const ExactFloat& b) {
  ...
  if (!a.is_normal() || !b.is_normal()) {
    ...
    ZETASQL_DCHECK(a.is_zero() || b.is_zero());
  ...
  BN_CTX* ctx = BN_CTX_new();
  ZETASQL_CHECK(BN_mul(r.bn_.get(), a.bn_.get(), b.bn_.get(), ctx));
  BN_CTX_free(ctx);
  r.Canonicalize();
  return r;
}
...
int ExactFloat::ScaleAndCompare(const ExactFloat& b) const {
  ZETASQL_DCHECK(is_normal() && b.is_normal() && bn_exp_ >= b.bn_exp_);
  ExactFloat tmp = *this;
  ZETASQL_CHECK(BN_lshift(tmp.bn_.get(), tmp.bn_.get(), bn_exp_ - b.bn_exp_));
...
ExactFloat ExactFloat::Unimplemented() {
  ZETASQL_LOG(FATAL) << "Unimplemented ExactFloat method called";
  return NaN();
}
