...
class FloatMargin {
 public:
  ...
  static T Ulp(T result) {
    // Using a CHECK instead of static_assert to make the code compile for
    // integer T.
    CHECK(!std::numeric_limits<T>::is_integer)
 ...
 private:
  ...
  T MaxAbsDiff(T x, T y) const {
    ...
    CHECK(ulp_bits_ >= 0 && ulp_bits_ <= kMaxUlpBits &&
    ...
    if (zero_ulp_bits_ > 0) {
      ...
      CHECK(std::isfinite(zero_margin)) << "Zero margin overflow: " << *this;
    ...
    CHECK(std::isfinite(result)) << "Float margin overflow: " << *this;
    return result;
  }
