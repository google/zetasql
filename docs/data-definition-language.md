...
+   `column_attribute`: A characteristic of the column. This can be:
    ...
    +  `[ CONSTRAINT constraint_name ] foreign_reference`: The column in a
       ...
       A constraint name must be unique within its schema; it cannot share the
...
+   `generated_column_expression`: A scalar expression that produces the
    content for the generated column. Subqueries are not allowed. This expression
    can:
    ...
    + Reference other generated columns, but recursion
      (or transitive recursion) is not permitted.

    + Call built-in functions.

    + Call UDFs that do not access SQL data.
+   `STORED`: Controls whether the column is pre-computed and persisted in
    storage when the row is inserted or updated.

    When this keyword is not present, the column is recomputed in
    ...
    that the value does not have to be recomputed each time it is read. That
...
By default, a generated column is not stored and the column is computed when
the row is read.
